...
//   const [photo, setPhoto] = useState('')
  ...
  const updateWish = () => {
    ...
    //   formData.append('photo' , photo && document.getElementById("photofile").files[0]);
    formData.append(
      'sponsorLogo',
      companyLogo && document.getElementById('companyLogo').files[0]
    )
    Axios({
      url: url,
      method: 'PUT',
          ...
          <div className="col-4">
            <div className="text-center">
              <img
                src={
                  wish.photoURL
                    ? 'http://localhost:3002/' + wish.photoURL
                    : Beemo
                }
                className="img-fluid rounded-circle shadow p-0 mb-5 bg-white rounded"
                alt="..."
              ></img>
            </div>
            <div className="row">
          ...
          <div className="col-3">
            <div className="card">
              <div className="card-body">
                ...
                ></img>
                <input
                  className={role === 'manager' ? 'inputFile' : 'hidden'}
                  id="companyLogo"
                  type="file"
                  onChange={x => setCompanyLogo(x.currentTarget.value)}
                  onClick={resetFile}
                />
              </div>
            </div>
            <div className="card">
              <div className="card-body">
                ...
                <form>
                  {' '}
                  {/*Figure out how to save sponsor id*/}
                  <div class="form-group">
                    {wish.orgId}
                    <input
                      type="string"
                      className={
                        role === 'manager' && !wish.orgId
                          ? 'form-control'
                          : 'hidden'
                      }
                      id="SponsorID"
                      placeholder="Enter Sponsor ID"
                      onChange={x => setOrgId(x.currentTarget.value)}
                    ></input>
                  </div>
                </form>
              </div>
            ...
            <div className="card">
              <div className="card-body">
                ...
                <input
                  ...
                  id="file"
                  type="file"
                  onChange={x => setVideo(x.currentTarget.value)}
                  onClick={resetFile}
                />
              </div>
            </div>
            ...
            <div className="card">
              <div className="card-body">
                ...
                <input
                  ...
                  id="file"
                  type="file"
                  onChange={x => setAudio(x.currentTarget.value)}
                  onClick={resetFile}
                />
              </div>
            </div>
          ...
          <div className="col">
            <div className="text-right">
              <div className={role === 'manager' ? 'visible' : 'invisible'}>
                ...
                {/*Button does not do anything atm*/}
              </div>
            </div>
          </div>
        </div>
